...
import { VIETNAMESE_WISH_TEXTS } from './vietnameseWishTemplates';
import { applyVietnameseAddress, resolveVietnameseAddress } from './vietnameseAddress';
...
export function getWishDeck(relationship: RelationshipType, name: string, language = 'en'): WishTemplate[] {
  ...
  const isVietnamese = ['vi', 'vn', 'vie', 'viet', 'vietnamese'].includes(language.toLowerCase().trim());
  const address = resolveVietnameseAddress(name);

  const interpolated = filtered.map(template => {
    const source = (isVietnamese && VIETNAMESE_WISH_TEXTS[template.id]) || template.text;
    const withAddress = isVietnamese ? applyVietnameseAddress(source, address) : source;
    return { ...template, text: withAddress.replace(/{name}/g, name) };
  });
