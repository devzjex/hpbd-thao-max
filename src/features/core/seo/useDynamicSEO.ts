...
export const useDynamicSEO = (config: BirthdayConfig) => {
  const { t, language, isBengali, isHindi, isFrench, isVietnamese } = useTranslation();
  ...
  useEffect(() => {
    ...
    const isVi = ['vi', 'vn', 'vie', 'viet', 'vietnamese'].includes(activeLanguage) || isVietnamese;
    ...
    if (rawName) {
      ...
      title = isVi
        ? "Birthday Bloom | Website Chúc Mừng Sinh Nhật Kỳ Diệu Đậm Chất Điện Ảnh"
        : isFr
    ...
    if (rawName) {
      if (isVi) {
        description = `Mừng sinh nhật ${rawName} bằng một trải nghiệm đậm chất điện ảnh: cắt bánh 3D, pháo hoa, thư viện ảnh kỷ niệm và những lời chúc từ tận đáy lòng trên Birthday Bloom.`;
      } else if (isFr) {
      ...
      description = isVi
        ? "Trải nghiệm một website chúc mừng sinh nhật đậm chất điện ảnh với hiệu ứng chuyển động, câu chuyện cảm xúc, thư viện ảnh riêng và những tương tác được làm bằng cả tấm lòng."
        : isFr
    ...
    const locale = isVi ? 'vi_VN' : isFr ? 'fr_FR' : isBn ? 'bn_BD' : isHi ? 'hi_IN' : 'en_US';
    ...
    document.documentElement.lang = isVi ? 'vi' : (activeLanguage || 'en');

  }, [config.name, config.age, config.relationship, config.senderName, config.language, language, t, isBengali, isHindi, isFrench, isVietnamese]);
};
