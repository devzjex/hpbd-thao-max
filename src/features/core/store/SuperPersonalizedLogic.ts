...
import { VIETNAMESE_EMOTIONAL_LETTERS, VIETNAMESE_BIG_WISHES } from '@/config/vietnameseTemplates';
...
export const getHighlySpecificLetter = (
    ...
    const isVietnamese = ['vi', 'vn', 'vie', 'viet', 'vietnamese'].includes(language);

    let letter = '';

    if (isVietnamese) {
        if (relationship === 'partner') {
            letter = isFemale ? VIETNAMESE_EMOTIONAL_LETTERS.partner.female(name) : VIETNAMESE_EMOTIONAL_LETTERS.partner.male(name);
        } else if (relationship === 'friend') {
            if (isFemale) letter = VIETNAMESE_EMOTIONAL_LETTERS.friend.friendly(name);
            else if (isMale) letter = VIETNAMESE_EMOTIONAL_LETTERS.friend.legend(name);
            else letter = VIETNAMESE_EMOTIONAL_LETTERS.friend.romantic(name);
        } else if (relationship === 'sibling') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.sibling(name);
        } else if (relationship === 'brother') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.brother(name);
        } else if (relationship === 'sister') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.sister(name);
        } else if (relationship === 'father') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.father(name);
        } else if (relationship === 'mother') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.mother(name);
        } else if (relationship === 'grandfather') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.grandfather(name);
        } else if (relationship === 'grandmother') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.grandmother(name);
        } else if (relationship === 'uncle') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.uncle(name);
        } else if (relationship === 'aunt') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.aunt(name);
        } else if (relationship === 'cousin') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.cousin(name);
        } else if (relationship === 'son') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.son(name);
        } else if (relationship === 'daughter') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.daughter(name);
        } else if (relationship === 'guardian') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.guardian(name);
        } else if (relationship === 'colleague') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.colleague(name);
        } else if (relationship === 'mentor') {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.mentor(name);
        } else {
            letter = VIETNAMESE_EMOTIONAL_LETTERS.family(name);
        }
    } else if (isFrench) {
        if (relationship === 'partner') {
    ...
    // Vietnamese letters carry a {ban} token for the second person; resolve it before
    // the signature placeholders so the register matches how the recipient is named.
    if (isVietnamese) {
        letter = applyVietnameseAddress(letter, resolveVietnameseAddress(name));
    }
    return letter
...
export const getBigWishes = (
    ...
    const isVietnamese = ['vi', 'vn', 'vie', 'viet', 'vietnamese'].includes(language);
    if (isVietnamese) {
        const address = resolveVietnameseAddress(name);
        return VIETNAMESE_BIG_WISHES(name, relationship, interests).map((entry) => ({
            ...entry,
            wish: applyVietnameseAddress(entry.wish, address),
        }));
    }
