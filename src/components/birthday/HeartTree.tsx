...
import { VIETNAMESE_SPECIAL_QUOTES } from "@/config/vietnameseTemplates";
import { useTranslation } from "@/i18n";

interface HeartTreeProps { delay?: number; }

const VIETNAMESE_HEART_MESSAGES = [
    "{Ban} là phần đáng nhớ nhất trong mọi kỷ niệm đáng giữ. ✨",
    "Giữa một thế giới toàn những khoảnh khắc bình thường, {ban} là điều đặc biệt. 🌸",
    "Mỗi năm {ban} lại rực rỡ hơn một chút, và điều đó vẫn khiến tôi bất ngờ. 🌟",
    "Ngay cả những vì sao cũng dịu đi một chút khi {ban} bước vào. 🌌",
    "Cảm ơn vì đã là lý do khiến căn phòng nào cũng ấm hơn. 🧡",
    "Tình bạn thật sự là có một người tử tế và nghĩa tình như {ban}. 🔥",
    "{Ban} mang sự tử tế như một siêu năng lực mà chính mình chẳng để ý. 💕",
    "Những lần {ban} lặng lẽ có mặt vì người khác là điều tôi luôn nhớ. 💫",
    "Nụ cười của {ban} là bí mật phía sau những khoảnh khắc vui nhất của tôi. 🌹",
    "{Ban} là minh chứng rằng điều đẹp nhất trong đời thường đến rất bất ngờ. ❤️",
    "Cười thật to, thương thật bền — đó là {ban}, luôn luôn thế. 🎉",
    "Mừng thêm một năm {ban} được sống đúng là chính mình. 💖",
];
...
export const HeartTree = ({ delay = 0 }: HeartTreeProps) => {
    ...
    const quotesPool = useMemo(() => {
        if (isVietnamese) {
            if (relationship === 'partner')
                return VIETNAMESE_SPECIAL_QUOTES.partner[gender as 'male' | 'female'] || VIETNAMESE_SPECIAL_QUOTES.family;
            if (relationship === 'friend')
                return (gender === 'male' ? VIETNAMESE_SPECIAL_QUOTES.friend.legend : VIETNAMESE_SPECIAL_QUOTES.friend.friendly) || VIETNAMESE_SPECIAL_QUOTES.family;
            if (relationship === 'brother' || relationship === 'sibling') return VIETNAMESE_SPECIAL_QUOTES.brother || VIETNAMESE_SPECIAL_QUOTES.family;
            if (relationship === 'sister') return VIETNAMESE_SPECIAL_QUOTES.sister || VIETNAMESE_SPECIAL_QUOTES.family;
            if (relationship === 'father') return VIETNAMESE_SPECIAL_QUOTES.father || VIETNAMESE_SPECIAL_QUOTES.family;
            if (relationship === 'mother') return VIETNAMESE_SPECIAL_QUOTES.mother || VIETNAMESE_SPECIAL_QUOTES.family;
            if (relationship === 'colleague') return VIETNAMESE_SPECIAL_QUOTES.colleague || VIETNAMESE_SPECIAL_QUOTES.family;
            if (relationship === 'mentor') return VIETNAMESE_SPECIAL_QUOTES.mentor || VIETNAMESE_SPECIAL_QUOTES.family;
            return VIETNAMESE_SPECIAL_QUOTES.family;
        }
        if (isFrench) {
            if (relationship === 'partner')
    ...
    const clickHeart = (e: React.MouseEvent<SVGGElement>, i: number) => {
        e.stopPropagation();
        if (stage < 3) return;
        const messages = isVietnamese ? VIETNAMESE_HEART_MESSAGES : isFrench ? FRENCH_HEART_MESSAGES : isBengali ? BENGALI_HEART_MESSAGES : isHindi ? HINDI_HEART_MESSAGES : HEART_MESSAGES;
        setActiveMsg(addr(messages[i] ?? quotesPool[i % quotesPool.length]));
        playPop();
