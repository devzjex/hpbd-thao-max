...
export const BirthdayQuiz = () => {
    ...
    const { addr, isHindi, isBengali, isFrench, isVietnamese } = useTranslation();
    ...
    const questions: Question[] = useMemo(() => {
        ...
        const displayName = name || (isVietnamese ? "Nhân Vật Chính Hôm Nay" : isFrench ? "Notre Star du Jour" : isBengali ? "আমাদের বার্থডে স্টার" : isHindi ? "हमारे बर्थडे स्टार" : "Birthday Star");
        if (isVietnamese) {
            const base: Question[] = [
                {
                    q: `Ai là người huyền thoại nhất sinh vào ngày hôm nay?`,
                    options: ["Albert Einstein", "Một người nổi tiếng", displayName, "Một chú chim cánh cụt"],
                    correct: 2,
                    reason: `Còn ai vào đây nữa! Chính là ${displayName}! Không ai sánh bằng.`
                },
                {
                    q: `Tâm trạng của ${displayName} hôm nay thế nào?`,
                    options: ["Buồn ngủ", "Đói bụng", "Bất bại & Huyền thoại 🔥", "Chán đời"],
                    correct: 2,
                    reason: "Sinh nhật mà! Hôm nay là chế độ ngôi sao luôn."
                }
            ];
            if (interests?.includes('car')) {
                base.push({
                    q: `Nếu hôm nay ${displayName} được lái bất kỳ chiếc xe nào, đó sẽ là:`,
                    options: ["Xe ba bánh", "Siêu xe gầm rú 🏎️", "Xe buýt", "Xe trượt scooter"],
                    correct: 1,
                    reason: "Vì huyền thoại thì phải lái thật ngầu!"
                });
            }
            if (relationship === 'partner') {
                base.push({
                    q: `Ai thương ${displayName} nhất trong cả vũ trụ này?`,
                    options: ["Con mèo", "Bác hàng xóm", "Người đã làm ra trang này ❤️", "Một người ngoài hành tinh"],
                    correct: 2,
                    reason: "Người gửi bất ngờ này thương nhiều hơn cả số sao trên trời!"
                });
            }
            if (interests?.includes('coding')) {
                base.push({
                    q: `Nỗi sợ lớn nhất của ${displayName} là gì?`,
                    options: ["Nhện", "Độ cao", "Bug production lúc 5h chiều thứ Sáu 🐞", "Hết cà phê"],
                    correct: 2,
                    reason: "Dân code hiểu mà... bug chính là cơn ác mộng lớn nhất!"
                });
            }
            base.push({
                q: `Nếu ${displayName} là siêu anh hùng, tên gọi sẽ là gì?`,
                options: ["Captain Ngủ Nướng", "Siêu Nhân Vui Vẻ ⚡", "Người Dơi Phiên Bản Lỗi", "Tiến sĩ Trì Hoãn"],
                correct: 1,
                reason: addr("Hôm nay {ban} chính là người hùng mà tất cả cần!")
            });
            return base;
        }
        if (isFrench) {
        ...
        return base;
    }, [config, addr, isHindi, isBengali, isFrench, isVietnamese]);
    ...
    return (<section className="relative z-20 px-4 py-20 max-w-4xl mx-auto" aria-label={isVietnamese ? "Đố vui sinh nhật" : "Birthday Celebration Trivia Quiz"}>
      <h2 className="sr-only">{isVietnamese ? "Đố vui sinh nhật" : "Birthday Celebration Trivia Quiz"}</h2>
        ...
        {!showResult ? (<AnimatePresence mode="wait">
            <motion.div key={currentIdx} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="space-y-8">
              <div className="flex justify-between items-center text-sm font-bold tracking-widest text-primary uppercase">
                <span>{isVietnamese ? `Câu ${currentIdx + 1} / ${questions.length}` : isFrench ? `Question ${currentIdx + 1} / ${questions.length}` : isBengali ? `প্রশ্ন ${currentIdx + 1} / ${questions.length}` : isHindi ? `प्रश्न ${currentIdx + 1} / ${questions.length}` : `Question ${currentIdx + 1} of ${questions.length}`}</span>
                <span>{isVietnamese ? `Điểm: ${score}` : isFrench ? `Score : ${score}` : isBengali ? `স্কোর: ${score}` : isHindi ? `स्कोर: ${score}` : `Score: ${score}`}</span>
              </div>
            ...
            <h2 className="font-display text-4xl md:text-6xl font-black">{isVietnamese ? "ĐIỂM SỐ HUYỀN THOẠI! 🏆" : isFrench ? "SCORE LÉGENDAIRE ! 🏆" : isBengali ? "অসাধারণ স্কোর! 🏆" : isHindi ? "धमाकेदार स्कोर! 🏆" : "LEGENDARY SCORE!"}</h2>
            <p className="text-2xl md:text-3xl text-foreground/80">
              {isVietnamese ? addr(`{Ban} được `) : isFrench ? `Vous avez obtenu ` : isBengali ? `আপনি ${config.name || 'বার্থডে'} কুইজে ` : isHindi ? `आपने ${config.name || 'बर्थडे'} क्विज़ में ` : 'You scored '}
              <span className="text-primary font-black">{score}/{questions.length}</span>
              {isVietnamese ? ` điểm trong bộ câu đố về ${config.name || 'nhân vật chính'}!` : isFrench ? ` au Quiz de ${config.name || 'Anniversaire'} !` : isBengali ? ' নম্বর পেয়েছেন!' : isHindi ? ' अंक हासिल किए!' : ` on the ${config.name || 'Birthday'} Trivia!`}
            </p>
            ...
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => {
              ...
              {isVietnamese ? "Chơi lại 🔄" : isFrench ? "Rejouer 🔄" : isBengali ? "আবার খেলুন 🔄" : isHindi ? "फिर से खेलें 🔄" : "Play Again 🔄"}
            </motion.button>
          </motion.div>)}
      </motion.div>
    </section>);
