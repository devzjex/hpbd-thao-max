...
export const CinematicIntro = ({ onComplete }: CinematicIntroProps) => {
    ...
    const storyLinesRaw = useMemo(() => {
        const isMale = gender === 'male';
        const isFemale = gender === 'female';
        if (isVietnamese) {
            if (relationship === 'partner') {
                return [
                    "Có một người đã trở thành *trung tâm cả thế giới của tôi*...",
                    "Một người biến mọi khoảnh khắc thành một *thước phim kỳ diệu*... ✨",
                    isMale ? "Người đàn ông mà *sự vững vàng và dịu dàng* đã thay đổi tất cả với tôi..." : isFemale ? "Người phụ nữ mà *sự dịu dàng và duyên dáng* làm sáng mọi khoảnh khắc..." : "Một tâm hồn tuyệt vời khiến tôi tin vào *điều kỳ diệu* mỗi ngày...",
                    "Tôi đã có thể chỉ nhắn một câu *'Thương {ban}'*...",
                    "Nhưng một tin nhắn thì chẳng bao giờ đủ để nói hết *những gì tôi đang cảm thấy* 💖"
                ];
            }
            if (relationship === 'friend') {
                return [
                    "Báo động toàn quốc! Một *huyền thoại thứ thiệt* vừa lên cấp! 🚀",
                    "Khoan đã, hôm nay sinh nhật {ban} thật hả hay cuốn lịch đang *chơi khăm* mình? 😂",
                    isMale ? "Gửi người chịu trách nhiệm cho *99% kỷ niệm đẹp nhất của tôi*..." : isFemale ? "Gửi người có nhiệm vụ *chọc tôi cười mỗi ngày*..." : "Gửi người *chất và ngầu nhất* trên đời này...",
                    "Tôi định tặng {ban} một món quà thật cổ điển và nghiêm túc...",
                    "Nhưng nghĩ lại... cái đó *đâu có giống tụi mình*! 😎🔥"
                ];
            }
            if (relationship === 'colleague' || relationship === 'mentor') {
                return [
                    "Có một người khiến mỗi ngày đi làm *nhẹ nhàng hơn hẳn*...",
                    "Người giữ được sự *bình tĩnh và tử tế* ngay cả những ngày deadline dí sát... 💼",
                    isMale ? "Người anh luôn *hỗ trợ mà chẳng cần ai nhờ*..." : isFemale ? "Người chị luôn *hỗ trợ mà chẳng cần ai nhờ*..." : "Người luôn *hỗ trợ mà chẳng cần ai nhờ*...",
                    "Tôi đã định chỉ nhắn một câu chúc mừng sinh nhật bình thường...",
                    "Nhưng một tin nhắn thì *chưa đủ* cho một người như vậy ✨"
                ];
            }
            return [
                "Hôm nay là một ngày sẽ được nhớ mãi như *một khoảnh khắc khó quên*...",
                "Vì đây là ngày một người rất đặc biệt *bước sang một tuổi mới*... ✨",
                "Người mang đến *hơi ấm và sự tử tế* cho tất cả những ai xung quanh...",
                "Tôi đã định chỉ gửi một lời chúc thật ngắn gọn...",
                "Nhưng một lời chúc thì *chẳng bao giờ đủ* 💖"
            ];
        }
        if (isFrench) {
            if (relationship === 'partner') {
    ...
    }, [relationship, gender, isHindi, isBengali, isFrench, isVietnamese]);
    const storyLines = useMemo(() => storyLinesRaw.map(addr), [storyLinesRaw, addr]);

    const postChatLinesRaw = useMemo(() => {
        if (isVietnamese) {
            if (relationship === 'friend')
                return [
                    "Vì {ban} không phải là một người bạn bình thường...",
                    "{Ban} là người tôi luôn có thể tin để *cười và nói đủ thứ chuyện trên đời*! ☕️",
                    "{Ban} xứng đáng một buổi tiệc *hoành tráng và độc nhất* như tình bạn này...",
                    "Vậy thì bắt đầu thôi và *quẩy đúng chất huyền thoại* nào! 🎉",
                ];
            if (relationship === 'partner')
                return [
                    "Nhưng {ban} còn hơn cả một người bạn đời với tôi...",
                    "{Ban} là *chốn bình yên*, là *chuyến phiêu lưu đẹp nhất* và là *nhà* của tôi.",
                    "Tôi đã thức cả đêm chăm chút từng chi tiết... *giống hệt cách {ban} vẫn làm* ✨",
                    "Sẵn sàng cho *màn công bố lớn* chưa? ❤️",
                ];
            if (relationship === 'colleague' || relationship === 'mentor')
                return [
                    "Vì {ban} không chỉ là một người đồng nghiệp...",
                    "{Ban} là người khiến cả team *dễ thở hơn* mỗi ngày ☕️",
                    "{Ban} xứng đáng một tuổi mới *nhẹ nhàng và trọn vẹn* hơn...",
                    "Vậy thì bắt đầu buổi tiệc thôi! 🎉",
                ];
            return [
                "{Ban} mang đến quá nhiều *ánh sáng và niềm vui* cho cuộc sống của chúng tôi...",
                "{Ban} xứng đáng một buổi tiệc *rực rỡ như nụ cười của {ban}*.",
                "Chúng tôi đã đặt vào đây *cả tấm lòng*, dành riêng cho {ban}...",
    ...
    }, [relationship, isHindi, isBengali, isFrench, isVietnamese]);
    const postChatLines = useMemo(() => postChatLinesRaw.map(addr), [postChatLinesRaw, addr]);

    const finalLinesRaw = useMemo(() => {
        if (isVietnamese) {
            if (relationship === 'partner')
                return [
                    `Gửi *${name || 'người thương'}* của tôi 💖`,
                    t('intro.hopeYouFeltHeartbeat'),
                    "{Ban} là *hôm nay*, *ngày mai* và *mãi mãi* của tôi ✨",
                    "Chúc mừng sinh nhật, *thương {ban} vô hạn*! 💖"
                ];
            if (relationship === 'friend')
                return [
                    `Chúc mừng sinh nhật *${name || 'huyền thoại'}*! 🚀`,
                    t('intro.cakeChaosNoRegrets'),
                    "Thật may vì có một *chiến hữu như {ban}* 🎉",
                    "Cứ *chất* như vậy nhé! 😎"
                ];
            if (relationship === 'colleague' || relationship === 'mentor')
                return [
                    `Chúc mừng sinh nhật *${name || 'bạn'}* 🎂`,
                    "Chúc một tuổi mới thật nhiều *năng lượng* và công việc *suôn sẻ*",
                    "*Ít họp hơn*, *ít việc gấp hơn* và thật nhiều thời gian cho những thứ {ban} thích",
                    "Chúc {ban} một tuổi mới *vui vẻ, khoẻ mạnh* và thật nhiều điều hay ho! 🥳"
                ];
            return [
                `Gửi *${name || 'người tuyệt vời'}* ✨`,
                t('intro.yearOfPureHappiness'),
                t('intro.kindnessReturnTenfold'),
                "Chúng tôi *quý {ban}* rất nhiều! 💖"
            ];
        }
        if (isFrench) {
            if (relationship === 'partner')
    ...
    }, [name, relationship, isHindi, isBengali, isFrench, isVietnamese, t]);
    const finalLines = useMemo(() => finalLinesRaw.map(addr), [finalLinesRaw, addr]);

    const effectiveStoryLines = useMemo(() => {
        if (!age) return storyLines;
        const ageLine = isVietnamese
            ? `Khi {ban} bước sang *tuổi ${age}* thật rực rỡ... ✨`
            : isFrench
        ...
        return [...storyLines.slice(0, -1), addr(ageLine), storyLines[storyLines.length - 1]];
    }, [age, storyLines, addr, isHindi, isBengali, isFrench, isVietnamese]);
                    ...
                    >
                        {revealStep === "dear-name" && (
                            ...
                            >
                                ...
                                >
                                    <KineticText
                                        text={name || (isVietnamese ? 'Bạn' : isFrench ? 'Vous' : isBengali ? 'আপনি' : isHindi ? 'आप' : 'You')}
                                        animation="zoom-in"
                                        delay={500}
                                    />
                                </h2>
