...
const CakeCard = ({ cake, onSelect }: {
    ...
    const { t, isHindi, isBengali, isFrench, isVietnamese } = useTranslation();
    const displayName = getCakeName(cake, isHindi, isBengali, isFrench, isVietnamese);
    return (
        <motion.button 
...
export const CakeCutting = () => {
    ...
    const quotesRaw = useMemo(() => {
        ...
        if (isVietnamese) {
            if (relationship === 'partner') return [
                { text: `${isMale ? 'Chàng trai' : isFemale ? 'Cô gái' : 'Người thương'} của tôi...`, animation: "zoom-in" as const },
                { text: "Ước một điều cho tương lai đẹp đẽ của chúng ta...", animation: "float" as const },
                { text: "Thương {ban} tới tận những vì sao và xa hơn nữa", animation: "pop-out" as const },
            ...
            if (relationship === 'friend') return [
                { text: `Ê ${name || 'bạn thân'} ơi!`, animation: "pop-out" as const },
            ...
            if (relationship === 'colleague' || relationship === 'mentor') return [
                { text: `Gửi ${name || 'bạn'}...`, animation: "zoom-in" as const },
                { text: "Chúc một tuổi mới thật nhiều năng lượng", animation: "float" as const },
                { text: "Công việc suôn sẻ, ít họp hơn, ít việc gấp hơn", animation: "stagger-up" as const },
                { text: `${t('common.happyBirthday')}!`, animation: "typewriter-burst" as const },
                { text: t('cake.celebrateYou'), animation: "float" as const },
            ];

            return [
                { text: `Gửi ${isMale ? 'người đàn ông' : isFemale ? 'người phụ nữ' : 'người'} tuyệt vời nhất của chúng tôi...`, animation: "zoom-in" as const },
    ...
    }, [name, relationship, gender, isHindi, isBengali, isFrench, isVietnamese, t]);
    const quotes = useMemo(() => quotesRaw.map((q) => ({ ...q, text: addr(q.text) })), [quotesRaw, addr]);
                        ...
                        >
                                        ...
                                        <div className="relative flex flex-col items-center gap-6">
                                            ...
                                            <div className="flex flex-col items-center gap-2">
                                                <h2 className="text-3xl md:text-4xl font-display font-black tracking-widest text-transparent bg-clip-text bg-gradient-to-r from-primary to-white uppercase">
                                                    {isVietnamese ? addr("Đang làm bánh cho {ban}...") : isFrench ? "Préparation de votre gâteau..." : isBengali ? "আপনার কেক তৈরি হচ্ছে..." : isHindi ? "आपका केक तैयार हो रहा है..." : "Baking Your Cake..."}
                                                </h2>
                            ...
                            <div className="w-full max-w-4xl mx-auto flex flex-col items-center justify-center min-h-[100dvh]">
                                ...
                                {phase === "quotes" && quoteIndex >= quotes.length - 1 && (
                                    ...
                                    >
                                        {isVietnamese ? "✕ Kết thúc trải nghiệm" : isFrench ? "✕ Terminer l'expérience" : isBengali ? "✕ অভিজ্ঞতা সমাপ্ত করুন" : isHindi ? "✕ अनुभव समाप्त करें" : "✕ Finish Experience"}
                                    </motion.button>
                                )}
                            </div>
                        </motion.div>
