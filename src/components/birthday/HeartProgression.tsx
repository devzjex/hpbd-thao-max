...
interface HeartProgressionProps {
    ...
    /** Stage 4 caption under the merged heart. Off where the surrounding copy already
        carries the sentiment, or where an affectionate line would read wrong. */
    showMessage?: boolean;
}
...
const FourCornerMerge = ({ onDone, showMessage = true }: {
    onDone: () => void;
    showMessage?: boolean;
}) => {
    ...
    const { isHindi, isBengali, isFrench, isVietnamese } = useTranslation();
    const loveMessage = isVietnamese
        ? `Thương ${name || 'bạn'} nhiều lắm`
        : isFrench
    ...
    return (<div ref={containerRef} className="relative flex flex-col items-center justify-center w-full h-full overflow-visible" style={{ minHeight: "300px" }}>
      ...
      {showText && showMessage && (<div className="z-40 mt-2 animate-love-text-reveal text-center px-4 max-w-full">
...
export const HeartProgression = ({ stage, onRevealComplete, showMessage = true }: HeartProgressionProps) => {
    if (stage === 4) {
        return (<div className="relative flex flex-col items-center justify-center w-full" style={{ minHeight: "200px" }}>
        <FourCornerMerge onDone={() => onRevealComplete?.()} showMessage={showMessage}/>
      </div>);
    }
