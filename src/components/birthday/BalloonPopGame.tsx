...
export const BalloonPopGame = () => {
    ...
    const wordsRaw = useMemo(() => {
        if (isVietnamese) {
            if (relationship === "partner") return ["{Ban}", "là", "cả", "thế giới! 💖"];
            if (relationship === "friend") return ["{Ban}", "đúng", "là", "huyền thoại! 🚀"];
            if (relationship === "brother" || relationship === "sibling") return ["{Ban}", "là", "số", "một! 🏆"];
            if (relationship === "sister") return ["{Ban}", "là", "cả", "phép màu! 🌸"];
            if (relationship === "father") return ["Bố", "là", "người", "hùng! 💪"];
            if (relationship === "mother") return ["Mẹ", "là", "trái", "tim nhà! 💐"];
            if (relationship === "grandfather" || relationship === "grandmother") return ["Người", "là", "niềm", "tự hào! 🏅"];
            if (relationship === "mentor" || relationship === "colleague") return ["{Ban}", "là", "nguồn", "cảm hứng! ✨"];
            return ["{Ban}", "là", "báu", "vật! 💝"];
        }
        if (isFrench) {
    ...
    const words = useMemo(() => wordsRaw.map(addr), [wordsRaw, addr]);
        ...
        <section className="relative z-20 py-20 px-4 max-w-5xl mx-auto w-full select-none">
            ...
            <div className="text-center mt-6 mb-12">
                ...
                <p className="font-display italic text-lg sm:text-2xl text-foreground/80">
                    {isVietnamese
                        ? "Chạm vào từng quả bóng để mở lời nhắn bí mật 💌"
                        : isFrench
                            ...
                            <AnimatePresence mode="wait">
                                {!balloon.isPopped ? (
                                    ...
                                    <motion.div
                                        ...
                                        className="flex flex-col items-center justify-center text-center px-3 py-4 rounded-3xl bg-white/5 border border-white/10 backdrop-blur-xl shadow-xl w-full min-h-40 overflow-hidden"
                                    >
                                        <span className="font-display italic text-2xl sm:text-3xl md:text-4xl font-black text-gradient-romantic text-glow-rose drop-shadow-xl break-words leading-snug max-w-full">
                                            {balloon.word}
                                        </span>
                                    </motion.div>
                                )}
