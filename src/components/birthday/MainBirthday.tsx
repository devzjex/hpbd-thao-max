...
import { YouTubeBgMusic } from "./YouTubeBgMusic";
...
export const MainBirthday = () => {
    ...
    const { playReveal, playPop, playBoom, setBgVolume, fadeOut } = useSoundManager();
    ...
    useEffect(() => {
        // The landing page has its own YouTube soundtrack — retire the intro track
        // so the two never overlap. Without a configured video we keep the intro BGM.
        if (YOUTUBE_BGM.videoId) {
            fadeOut(1500);
        }
        else {
            setBgVolume(0.4);
    ...
    }, [playReveal, playBoom, setBgVolume, fadeOut, fireCannon]);
    const addEmoji = () => {
    ...
    return (<div onMouseMove={shouldAnimate ? handleMouseMove : undefined} className={`min-h-screen transition-opacity duration-1000 w-full max-w-[100vw] overflow-x-hidden ${visible ? "opacity-100" : "opacity-0"} ${megaSurprise ? "animate-screen-shake" : ""}`} style={{ background: 'transparent' }}>
      ...
      <motion.header initial="hidden" animate="visible" variants={containerVariants} className="relative min-h-screen flex flex-col items-center justify-center text-center px-4 py-20 overflow-hidden">
        ...
        <motion.div variants={itemVariants} className="mb-6 relative z-10">
          <div className="flex justify-center mb-8"><HeartProgression stage={4} showMessage={false}/></div>
      ...
      <section className="relative z-20 flex flex-wrap justify-center gap-4 sm:gap-6 px-4 pb-32">
        {[
            ...
            ...(config.showShareSection === false
                ? []
                : [{ label: isVietnamese ? "💌 Chia sẻ!" : isFrench ? "💌 Partager!" : isBengali ? "💌 শেয়ার!" : isHindi ? "💌 शेयर!" : "💌 Share!", color: "hsl(320, 85%, 55%)", action: () => setShareOpen(true) }])
      ...
      <footer className="relative z-20 text-center py-20 bg-gradient-to-t from-black/60 to-transparent w-full">
        {config.showShareSection !== false && <div className="flex items-center justify-center gap-2 mb-4">
          <button
            type="button"
          ...
          >
            ...
            <span>{isVietnamese ? addr("Chia sẻ hoặc Tạo bất ngờ của {ban} ✨") : isFrench ? "Partager ou Créer une Surprise ✨" : isBengali ? "সারপ্রাইজ শেয়ার বা নতুন তৈরি করুন ✨" : isHindi ? "सरप्राइज शेयर करें या नया बनाएं ✨" : "Share Surprise or Create Yours ✨"}</span>
          </button>
        </div>}
        <p className="text-white/30 text-sm tracking-[0.15em]">
          {isVietnamese ? "Thân gửi từ" : isFrench ? "Avec amour de" : isBengali ? "ভালোবাসায়" : isHindi ? "प्यार से" : "With love from"}{' '}
