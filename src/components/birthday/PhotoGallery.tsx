...
export const PhotoGallery = () => {
    ...
    const photos = useMemo(() => {
        ...
        const captions = isVietnamese ? (relationship === 'partner' ? [
            "Mỗi khoảnh khắc bên {ban} đều là một món quà 💖",
            "Cùng nhau xây một tương lai thật đẹp ✨",
            "Nơi trái tim tôi muốn quay về nhất 🌹"
        ] : relationship === 'friend' ? [
            "Những ngày huyền thoại cùng chiến hữu 🚀",
            "Đầy ắp kỷ niệm và những quyết định khó đỡ! 😂",
            "Cứ chất và cứ là chính mình nhé! 🍻"
        ] : relationship === 'colleague' || relationship === 'mentor' ? [
            "Những ngày cùng nhau chạy deadline 💼",
            "Cảm ơn vì đã luôn đồng hành cùng team ✨",
            "Chúc một tuổi mới thật nhiều năng lượng 🌟"
        ] : [
            "Gia đình là nơi mọi hành trình bắt đầu ✨",
            "Trân trọng từng nụ cười 💖",
            "Một hành trình ngập tràn yêu thương 🌟"
        ]) : isFrench ? (relationship === 'partner' ? [
        ...
        return validPhotos.map((src, i) => ({
            src,
            key: `photo-${i}`,
            caption: config.photoCaptions?.[i] || addr(captions[i % captions.length]) || (isVietnamese ? "Kỷ niệm đẹp" : isFrench ? "Doux souvenir" : isBengali ? "সুন্দর স্মৃতি" : isHindi ? "खूबसूरत याद" : "Beautiful memory"),
        }));
    }, [validPhotos, relationship, config.photoCaptions, addr, isHindi, isBengali, isFrench, isVietnamese]);
            ...
            <section className="relative z-20 px-4 py-24 max-w-4xl mx-auto overflow-hidden">
                ...
                >
                    ...
                    <p className="text-base sm:text-xl text-foreground/80 font-light max-w-xl mx-auto leading-relaxed mb-6">
                        {isVietnamese
                            ? "Ở đây chưa có tấm ảnh nào của chúng ta, nhưng chắc chắn sẽ còn thật nhiều kỷ niệm đáng nhớ phía trước!"
                            : isFrench
                    ...
                    <div className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/5 border border-white/10 text-xs sm:text-sm text-white/60">
                        ...
                        <span>{isVietnamese ? "Còn nhiều chuyến phiêu lưu đang chờ ✨" : isFrench ? "De nouvelles aventures à venir ✨" : isBengali ? "সামনে আসছে দারুণ সব স্মৃতি ✨" : isHindi ? "ढेर सारी नई यादें आने वाली हैं ✨" : "Future adventures await ✨"}</span>
                    </div>
                </motion.div>
            </section>
        );
    ...
    return (<>
      <section className="relative z-20 px-4 py-32 max-w-7xl mx-auto overflow-hidden" aria-label={isVietnamese ? "Thư viện ảnh kỷ niệm" : "Photo Memories Gallery"}>
        ...
        <motion.div onMouseMove={handleMouseMove} onMouseLeave={handleMouseLeave} style={{ rotateX, rotateY, perspective: 1000 }} className={`relative group ${isMobile ? '' : 'cursor-none'}`}>
          ...
          {photos.length > 1 && (
            <div className="absolute inset-0 flex items-center justify-between px-8 opacity-0 group-hover:opacity-100 transition-all duration-500">
              <button aria-label={isVietnamese ? "Ảnh trước" : "Previous photo"} onClick={(e) => { e.stopPropagation(); setActiveIndex((activeIndex - 1 + photos.length) % photos.length); }} className="w-20 h-20 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 flex items-center justify-center text-white text-4xl hover:bg-primary transition-all shadow-2xl">
                ‹
              </button>
              <button aria-label={isVietnamese ? "Ảnh tiếp theo" : "Next photo"} onClick={(e) => { e.stopPropagation(); setActiveIndex((activeIndex + 1) % photos.length); }} className="w-20 h-20 rounded-full bg-black/40 backdrop-blur-xl border border-white/10 flex items-center justify-center text-white text-4xl hover:bg-primary transition-all shadow-2xl">
                ›
              </button>
            </div>
          )}
