...
export const EnvelopeLetterScene = ({
    ...
    const { addr, isHindi, isBengali, isFrench, isVietnamese, language } = useTranslation();
    ...
    const paragraphs = (() => {
        ...
        const generated = getHighlySpecificLetter(
            config.name || (isVietnamese ? "Người thương mến" : isFrench ? "Mon Amour" : isBengali ? "প্রিয়" : isHindi ? "प्रिय" : "My Dearest"),
            relationship,
        ...
        if (isVietnamese) {
            if (relationship === "partner") {
                return [
                    `{Ban} là sự dịu dàng, là sức mạnh và là chỗ dựa của tôi. Tôi biết ơn vô cùng vì có {ban} trong đời. 💖`,
                    `{Ban} mang đến quá nhiều hơi ấm và ánh sáng cho những ngày bình thường của tôi. Mỗi khoảnh khắc bên {ban} đều là một điều quý giá.`,
                    `Trong ngày đặc biệt này, tôi chúc {ban} tất cả hạnh phúc, yêu thương và niềm vui mà trái tim {ban} xứng đáng. Mong năm nay mang tới thật nhiều kỷ niệm đẹp. ✨`,
                    `Gửi tình cảm của chúng ta, hôm nay và mãi về sau. 🌹`,
                ];
            }
            if (relationship === "friend") {
                return [
                    `Chúc mừng sinh nhật người chiến hữu tuyệt vời nhất trên đời! 🚀`,
                    `Cảm ơn vì tất cả những trận cười, những chuyến đi đáng nhớ và vì đã luôn ở đó.`,
                    `Chúc {ban} một năm thật hoành tráng, đầy thành công, bánh kem và kỷ niệm khó quên! 🎉🍻`,
                ];
            }
            if (relationship === "colleague" || relationship === "mentor") {
                return [
                    `Làm việc cùng {ban} là một trong những điều khiến mỗi ngày đi làm nhẹ nhàng hơn hẳn. 💼`,
                    `Cảm ơn vì những lần hỗ trợ không cần ai nhờ, những góp ý thẳng thắn mà vẫn ấm áp, và vì đã giữ cho không khí team luôn dễ chịu.`,
                    `Chúc {ban} một tuổi mới thật nhiều năng lượng, công việc suôn sẻ, ít họp hơn, ít việc gấp hơn và thật nhiều thời gian cho những thứ mình thích. ✨`,
                ];
            }
            return [
                `Gửi một người đặc biệt, người có sự tử tế và rộng lượng làm sáng cả thế giới quanh mình. 🌟`,
                `Cảm ơn vì tất cả yêu thương, sự quan tâm và sự hiện diện ấm áp ấy.`,
                `Mong ngày hôm nay tuyệt vời và quý giá đúng như {ban} vẫn thế với tất cả chúng tôi! 💝`,
            ];
        }
                ...
                {!isExtracted ? (
                    ...
                    >
                        ...
                        <div className="relative w-[310px] sm:w-[380px] h-[210px] sm:h-[250px] rounded-3xl bg-[#FAF6F0] shadow-[0_25px_60px_-15px_rgba(0,0,0,0.8),0_0_30px_rgba(255,200,120,0.2)] border-2 border-[#E5C378] overflow-hidden flex items-center justify-center">
                            ...
                            >
                                <p className="font-script text-xs sm:text-sm text-[#8B5A2B] opacity-70">
                                    {isVietnamese ? "Dành cho {ban}..." : isFrench ? "Pour toi..." : isBengali ? "তোমার জন্য..." : isHindi ? "आपके लिए..." : "For you..."}
                                </p>
                            </motion.div>
                        </div>
                        ...
                        <p className="font-display text-xs sm:text-sm tracking-widest uppercase text-white/50 mt-4 flex items-center gap-2">
                            ...
                            {isOpen
                                ? (isVietnamese ? "Đang mở thư..." : isFrench ? "Ouverture de la lettre..." : isBengali ? "চিঠি খোলা হচ্ছে..." : isHindi ? "पत्र खुल रहा है..." : "Opening letter...")
                                : (isVietnamese ? "Chạm để mở thư 💌" : isFrench ? "Appuyez pour ouvrir 💌" : isBengali ? "চিঠি খুলতে স্পর্শ করুন 💌" : isHindi ? "खोलने के लिए टैप करें 💌" : "Tap to open letter 💌")}
                        </p>
                    </motion.div>
                ) : (
                    ...
                    >
                        ...
                        <div className="relative z-10 mt-8 pt-4 border-t border-[#D4AF37]/35 flex items-center justify-between">
                            <div className="font-script text-lg sm:text-xl md:text-2xl text-[#5C3215] font-bold tracking-wide">
                                {senderName ? `— ${senderName}` : (isVietnamese ? `— Thân mến 💕` : `— Yours Forever 💕`)}
                            </div>
                        ...
                        {onComplete && isTypingDone && (
                            ...
                            >
                                ...
                                >
                                    <span>{isVietnamese ? "Tiếp tục buổi tiệc ✨" : isFrench ? "Continuer la Fête ✨" : isBengali ? "উদযাপনে এগিয়ে যান ✨" : isHindi ? "जश्न जारी रखें ✨" : "Continue Celebration ✨"}</span>
                                    <ArrowRight size={17} />
                                </button>
                            </motion.div>
                        )}
