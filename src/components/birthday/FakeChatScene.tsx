...
/** Target wall-clock for typing out the closing message. */
const TYPING_BUDGET_MS = 5500;
...
export const FakeChatScene = ({ onComplete }: FakeChatSceneProps) => {
    ...
    const { t, addr, isHindi, isBengali, isFrench, isVietnamese } = useTranslation();
    ...
    const heartfeltTextRaw = useMemo(() => {
        // An explicit VITE_CHAT_MESSAGE wins over every generated variant.
        const configured = config.chatMessage?.trim();
        if (configured) return configured;

        if (isVietnamese) {
            if (relationship === "partner")
                return isMale ? "Gửi người đàn ông giữ trái tim tôi... ❤️" : isFemale ? "Gửi người phụ nữ trong mơ của tôi... ❤️" : "Gửi tâm hồn khiến tôi trọn vẹn... ❤️";
            if (relationship === "friend")
                return "Khoan đã, nhắn tin khô khan vậy? Không giống tụi mình! 😂🚀";
            if (relationship === "brother" || relationship === "sibling")
                return "Nhắn tin bình thường cho người anh em tuyệt nhất vũ trụ? Không đời nào! 🏆";
            if (relationship === "sister")
                return "Một tin nhắn khô khan cho chị em gái dễ thương nhất? {Ban} xứng đáng cả một vũ trụ! 🌸✨";
            if (relationship === "father")
                return "Một tin nhắn thì làm sao cảm ơn hết người hùng của con... 🌟💪";
            if (relationship === "mother")
                return "Một tin nhắn thì không bao giờ nói hết được hơi ấm mẹ dành cho con... 💐💛";
            if (relationship === "mentor" || relationship === "colleague")
                return "Chúc mừng sinh nhật kiểu thường thôi hả? Với một nguồn cảm hứng như vậy thì phải khác chứ! 🎯✨";
            return "Một tin nhắn thì không bao giờ nói hết {ban} quan trọng thế nào với cả nhà... 💝🌟";
        }
        if (isFrench) {
            if (relationship === "partner")
    ...
    }, [relationship, isMale, isFemale, isHindi, isBengali, isFrench, isVietnamese, config.chatMessage]);
    const heartfeltText = useMemo(() => addr(heartfeltTextRaw), [heartfeltTextRaw, addr]);
    ...
    useEffect(() => {
        ...
        const runSequence = async () => {
            ...
            // Hold the whole type-out to roughly TYPING_BUDGET_MS so a long, heartfelt
            // message does not drag the scene out; short ones keep the original pace.
            const heartfeltDelay = Math.max(
                18,
                Math.min(55, TYPING_BUDGET_MS / Math.max(1, heartfeltText.length))
            );
            ...
            >
                ...
                <header className="relative z-20 px-3 py-2 sm:px-4 sm:py-2.5 flex items-center justify-between border-b border-white/10 bg-[#160614]/80 backdrop-blur-xl shrink-0">
                    ...
                    <div className="flex items-center gap-2.5">
                        <button
                            type="button"
                            aria-label={isVietnamese ? "Quay lại" : "Go back"}
                    ...
                    <div className="flex items-center gap-1 text-white/80">
                        <button
                            type="button"
                            aria-label={isVietnamese ? "Gọi thoại" : "Voice call"}
                        ...
                        <button
                            type="button"
                            aria-label={isVietnamese ? "Gọi video" : "Video call"}
                            className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 active:scale-95 transition-all text-white/80 hover:text-white"
                        >
                            <Video size={21} />
                        </button>
                        <button
                            type="button"
                            aria-label={isVietnamese ? "Thông tin đoạn chat" : "Chat details"}
                            className="w-10 h-10 flex items-center justify-center rounded-full hover:bg-white/10 active:scale-95 transition-all text-white/80 hover:text-white"
                        >
                            <Info size={20} />
                        </button>
                ...
                <main className="relative z-10 flex-1 px-4 py-4 flex flex-col justify-end gap-3.5 overflow-y-auto">
                    ...
                    {/* Outgoing opener — these are the sender's own words, so they sit
                        on the right like any message you'd type yourself. The muted
                        gradient keeps the vivid one below as the climax. */}
                    <motion.div
                        initial={{ opacity: 0, x: 16, scale: 0.94 }}
                        animate={{ opacity: 1, x: 0, scale: 1 }}
                        transition={{ duration: 0.35, ease: "easeOut" }}
                        onClick={handleDoubleTapMessage}
                        className="relative self-end max-w-[82%] px-4 py-3 rounded-[20px] rounded-br-[4px] backdrop-blur-md text-white text-sm sm:text-[15px] font-normal shadow-md border border-white/15 leading-relaxed cursor-pointer active:scale-[0.98] transition-transform"
                        style={{ background: "linear-gradient(135deg, rgba(131,58,180,0.55) 0%, rgba(253,29,29,0.4) 60%, rgba(252,176,69,0.35) 100%)" }}
                    >
                        <span>
                    ...
                    {/* Outgoing teaser — the recipient hearts this one, so the reaction
                        badge hangs off the inner edge of an outgoing bubble. */}
                    <motion.div
                        initial={{ opacity: 0, x: 16, scale: 0.94 }}
                        animate={{ opacity: 1, x: 0, scale: 1 }}
                        transition={{ delay: 0.3, duration: 0.35, ease: "easeOut" }}
                        onClick={handleDoubleTapMessage}
                        className="relative self-end max-w-[85%] px-4 py-3 rounded-[20px] rounded-br-[4px] backdrop-blur-md text-white text-sm sm:text-[15px] font-normal shadow-md border border-white/15 leading-relaxed cursor-pointer active:scale-[0.98] transition-transform"
                        style={{ background: "linear-gradient(135deg, rgba(131,58,180,0.55) 0%, rgba(253,29,29,0.4) 60%, rgba(252,176,69,0.35) 100%)" }}
                    >
                        <span>
                            {relationship === "partner"
                        ...
                        <AnimatePresence>
                            {isHeartLiked && (
                                <motion.div
                                    ...
                                    className="absolute -bottom-2.5 -left-1.5 px-2 py-0.5 rounded-full bg-[#1C1C20] border border-white/15 flex items-center shadow-lg cursor-pointer hover:scale-110 active:scale-95 transition-transform"
                                >
                    ...
                    {/* Incoming reply — the birthday person answering back. This is the
                        only received bubble in the thread. */}
                    <motion.div
                        initial={{ opacity: 0, x: -16, scale: 0.94 }}
                        animate={{ opacity: 1, x: 0, scale: 1 }}
                        transition={{ delay: 1.15, duration: 0.35, ease: "easeOut" }}
                        className="relative self-start max-w-[82%] px-4 py-3 rounded-[20px] rounded-bl-[4px] bg-[#26262B]/90 backdrop-blur-md text-white text-sm sm:text-[15px] font-normal shadow-md border border-white/10 leading-relaxed"
                    >
                        <span>
                            {relationship === "partner"
                                ? t("chat.replyLove")
                    ...
                    <AnimatePresence>
                        {sentMessageText && (
                            ...
                            >
                                <div
                                    className="relative px-4 py-3 rounded-[20px] rounded-br-[4px] text-white text-sm sm:text-[15px] font-normal shadow-[0_8px_24px_rgba(236,72,153,0.35)] leading-relaxed border border-white/15 whitespace-pre-line"
                                    style={{
                ...
                >
                    <div className="flex items-center gap-2">
                        ...
                        <button
                            type="button"
                            aria-label={isVietnamese ? "Mở máy ảnh" : "Open camera"}
                        ...
                        <div className="flex-1 rounded-full px-3.5 py-2 bg-[#26262B] border border-white/10 flex items-center justify-between min-h-[44px] overflow-hidden">
                            ...
                            <div className="flex items-center gap-0.5 text-white/50 shrink-0">
                                <button type="button" aria-label={isVietnamese ? "Ghi âm" : "Record voice note"} className="w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
                                    <Mic size={18} />
                                </button>
                                <button type="button" aria-label={isVietnamese ? "Đính kèm ảnh" : "Attach photo"} className="w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
                                    <ImageIcon size={18} />
                                </button>
                                <button type="button" aria-label={isVietnamese ? "Thêm sticker hoặc emoji" : "Add sticker or emoji"} className="w-8 h-8 flex items-center justify-center hover:text-white transition-colors">
                                    <Smile size={18} />
                                </button>
                            </div>
                        </div>
                        ...
                        <button
                            type="button"
                            aria-label={isVietnamese ? "Gửi tin nhắn" : "Send message"}
                        ...
                        >
                            ...
                            <p className="text-xl sm:text-2xl font-display font-black leading-snug bg-gradient-to-r from-white via-rose-200 to-amber-200 bg-clip-text text-transparent drop-shadow-[0_0_25px_rgba(255,255,255,0.4)]">
                                {relationship === "partner"
                                    ? isVietnamese
                                        ? isMale
                                            ? addr("Vì một chàng trai như {ban} xứng đáng nhiều hơn một tin nhắn... ✨")
                                            : addr("Vì một cô gái như {ban} xứng đáng nhiều hơn một tin nhắn... ✨")
                                        : isFrench
                                        ? isMale
